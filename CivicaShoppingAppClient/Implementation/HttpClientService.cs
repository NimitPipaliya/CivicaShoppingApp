...
namespace CivicaShoppingAppClient.Implementation
{
    ...
    public class HttpClientService : IHttpClientService
    {
        ...
        public T ExecuteApiRequest<T>(string url, HttpMethod method, HttpRequest httpRequest, object? parameters = null, int? TimeOutInSecond = 60) where T : class
        {
            ...
            var responsContent = string.Empty;
            if (response.IsSuccessStatusCode)
            {
                responsContent = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }
            else
            {
                responsContent = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            ...
            return JsonConvert.DeserializeObject<T>(responsContent);
        }
